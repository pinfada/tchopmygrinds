...
const RatingSummary: React.FC<RatingSummaryProps> = ({
  ...
  if (!stats || typeof stats.totalRatings !== 'number' || typeof stats.averageRating !== 'number') {
    return (
  ...
  const verifiedRatings = stats.verifiedRatings ?? 0
  ...
  const getRatingColor = (rating: number) => {
    ...
    if (rating >= 3.5) return 'text-accent-500'
    if (rating >= 3) return 'text-accent-700'
  ...
  return (
    <div className={`${config.spacing} ${className}`}>
      ...
      {/* Badge de confiance */}
      {verifiedRatings / stats.totalRatings >= 0.5 && (
